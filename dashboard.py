...
def _render_revision_capture(trace_data: dict) -> None:
    """Inline conversational revision + observer flow for one trace (V1).

    Replaces V0's binary confirm/reject with a multi-round conversation:
      1. Edit + comment → propose_hypothesis (round 1)
      2. Display structured hypothesis (principle / applies_when /
         doesnt_apply_when / specific_rule / clarifying_questions)
      3. Lior responds → refine_hypothesis (round 2+)
      4. At any point: accept current hypothesis, OR mark as rejected.

    Streamlit session state keys per trace_id:
      revdone_<id>      : True after final accept/reject (terminal)
      revid_<id>        : str — the revision record id, persists across reruns
      hyp_<id>          : dict — current displayed hypothesis (latest round)
    """
    ...
    st.markdown("---\n#### ✍️ Teach the agent")
    st.caption(
        "Edit the output. The observer hypothesizes ONE preference rule and "
        "asks a clarifying question. Push back, refine, repeat — converge on "
        "a rule with the right context. Confirmed rules land in "
        "`voice_corpus/lior_preferences.md` and future drafts honor them."
    )

    # Source text the user edits
    ...
    if hyp is None:
        ...
        with run_col:
            run_obs = st.button(
                "🔍 Run observer (round 1)",
                key=f"runobs_{trace_id}",
                help="Observer reads your edit and proposes a structured rule with context. ~$0.01.",
                type="primary",
            )
        if run_obs:
    ...
    # ---- Stage 3: display + refine -----------------------------------------
    round_n = hyp.get("_round", 1)
    revision_id = hyp.get("_revision_id", "")

    st.markdown(f"---\n#### 💡 Observer's hypothesis — round {round_n}")
    ...
    if default_scope not in scope_options:
        default_scope = "other"
    scope = st.selectbox(
        "Scope (you can override the observer's guess)",
        scope_options,
        index=scope_options.index(default_scope),
        key=f"scope_{trace_id}_{round_n}",
    )

    principle = st.text_area(
        "Principle — the underlying why",
        value=hyp.get("principle", ""),
        height=80,
        key=f"principle_{trace_id}_{round_n}",
    )

    aw_list = hyp.get("applies_when", []) or []
    nw_list = hyp.get("doesnt_apply_when", []) or []
    applies_when_raw = st.text_area(
        "Applies when (one context per line)",
        value="\n".join(aw_list),
        height=80,
        key=f"aw_{trace_id}_{round_n}",
    )
    doesnt_apply_raw = st.text_area(
        "Does NOT apply when (one context per line)",
        value="\n".join(nw_list),
        height=60,
        key=f"nw_{trace_id}_{round_n}",
    )

    specific_rule = st.text_area(
        "Specific rule — what the drafter actually does",
        value=hyp.get("specific_rule", ""),
        height=80,
        key=f"rule_{trace_id}_{round_n}",
    )

    if hyp.get("reasoning"):
        with st.expander("Observer's reasoning", expanded=False):
            st.markdown(hyp["reasoning"])

    # Clarifying question — surfaced prominently as the prompt for next round
    questions = hyp.get("clarifying_questions") or []
    if questions:
        st.markdown("**💬 Observer asks:**")
        for q in questions:
            st.markdown(f"> {q}")

    response = st.text_area(
        "Your response / refinement (will be sent to the observer if you click Refine)",
        value="",
        height=80,
        key=f"resp_{trace_id}_{round_n}",
        placeholder="e.g. 'only for cold first touches' · 'the principle is X, not Y'",
    )

    b1, b2, b3, b4 = st.columns(4)
    with b1:
        if st.button(f"🔁 Refine (round {round_n + 1})", key=f"refine_{trace_id}_{round_n}",
                     help="Send your response back to the observer for another pass. ~$0.01."):
            if not response.strip():
                st.warning("Add a response first — empty refinement won't change the rule.")
            else:
                with st.spinner("Observer revising…"):
                    new_hyp = refine_hypothesis(
                        revision_id=revision_id,
                        lior_response=response.strip(),
                    )
                st.session_state[hyp_key] = new_hyp
                st.rerun()
    with b2:
        if st.button("✅ Accept this version", key=f"accept_{trace_id}_{round_n}",
                     type="primary",
                     help="Save the rule (with your edits) to voice_corpus/lior_preferences.md."):
            try:
                final = {
                    "principle": principle.strip(),
                    "applies_when": [ln.strip() for ln in applies_when_raw.splitlines() if ln.strip()],
                    "doesnt_apply_when": [ln.strip() for ln in doesnt_apply_raw.splitlines() if ln.strip()],
                    "specific_rule": specific_rule.strip(),
                    "confidence": conf,
                    "scope": scope,
                }
                accept_hypothesis(revision_id=revision_id, final_hypothesis=final)
    ...
    with b3:
        if st.button("⏭ Just this case", key=f"skip_{trace_id}_{round_n}",
                     help="Save the revision as data but don't generalize into a rule."):
            try:
                reject_revision(revision_id, reason="just_this_case")
                st.session_state[done_key] = True
                st.session_state.pop(hyp_key, None)
                ...
                st.error(f"Could not save: {e}")
    with b4:
        if st.button("❌ Wrong inference", key=f"reject_{trace_id}_{round_n}",
                     help="The observer's read was off. Tunes the observer's prompt over time."):
            try:
                reject_revision(revision_id, reason="wrong_inference")
                st.session_state[done_key] = True
                st.session_state.pop(hyp_key, None)
